...
from pybuilder import core, extern, pip_utils
...
class PipVersionTests(unittest.TestCase):
    def test_pip_dependency_version(self):
        self.assertEqual(
            pip_utils.build_dependency_version_string(core.Dependency("test", "1.2.3")),
            ">=1.2.3",
        )
        self.assertEqual(
            pip_utils.build_dependency_version_string(
                core.Dependency("test", ">=1.2.3,<=2.3.4")
            ),
            "<=2.3.4,>=1.2.3",
        )
    ...
    def test_version_satisfies_spec(self):
        ...
        self.assertEqual(
            pip_utils.version_satisfies_spec(">=1.2.3", "1.2.4.dev987"), False
        )
    ...
    def test_get_package_version(self):
        ...
        self.assertTrue(
            pip_utils.version_satisfies_spec(
                ">=7.0", pip_utils.get_package_version("pip")["pip"]
            )
        )
        self.assertTrue(
            "this package does not exist"
            not in pip_utils.get_package_version("this package does not exist")
        )
        self.assertTrue(
            "blah" not in pip_utils.get_package_version(core.RequirementsFile("blah"))
        )
        self.assertTrue(
            "blah"
            not in pip_utils.get_package_version(
    ...
    def test_build_pip_install_options(self):
        ...
        self.assertEqual(
            pip_utils.build_pip_install_options(index_url="foo"), ["--index-url", "foo"]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            pip_utils.build_pip_install_options(index_url="foo", extra_index_url="bar"),
            ["--index-url", "foo", "--extra-index-url", "bar"],
        )
        self.assertEqual(
            pip_utils.build_pip_install_options(extra_index_url=("foo", "bar")),
        ...
        self.assertEqual(
            pip_utils.build_pip_install_options(upgrade=True, eager_upgrade=True),
        ...
        self.assertEqual(
            pip_utils.build_pip_install_options(constraint_file="constraint file"),
            ["-c", "constraint file"],
        )
        self.assertEqual(
            pip_utils.build_pip_install_options(insecure_installs=["foo", "bar"]),
            [
                "--allow-unverified",
                "foo",
                "--allow-external",
