...
from pybuilder import pip_common, pluginloader
from pybuilder.core import Dependency, PluginDef
from pybuilder.errors import IncompatiblePluginException, MissingPluginException
...
class DownloadingPluginLoaderTest(unittest.TestCase):
    ...
    @patch("pybuilder.pluginloader._load_plugin")
    def test_should_not_load_module_twice_after_downloading_when_vcs_download_fails(
        self, load
    ):
        logger = Mock()
        reactor = Mock()
    ...
    @patch("pybuilder.pluginloader._load_plugin")
    def test_should_load_module_after_downloading_with_vcs_when_download_succeeds(
        self, load
    ):
        logger = Mock()
        reactor = Mock()
        ...
        downloader = DownloadingPluginLoader(logger)
        pd = PluginDef(
            "vcs:external_plugin URL", plugin_module_name="external_plugin_module"
        )
        plugin = downloader.load_plugin(reactor, pd)
    ...
    def test_should_raise_exception_when_requiring_plugin_and_plugin_is_not_found(
        ...
        self.assertRaises(
            MissingPluginException, downloader.load_plugin, project, PluginDef("spam")
        )
    ...
    def test_should_import_plugin_when_requiring_plugin_and_plugin_is_found_as_third_party(
        ...
        self.assertEqual(
            load.return_value, downloader.load_plugin(reactor, PluginDef("spam"))
        )
    ...
    def test_should_raise_error_when_protocol_is_invalid(self):
        ...
        self.assertRaises(
            ...
            downloader.install_plugin,
            reactor,
            PluginDef("some-plugin"),
        )
    ...
    def test_should_install_plugin(self):
        ...
        pyb_env.install_dependencies.assert_called_with(
            [pd.dependency], package_type="plugin"
        )
...
class BuiltinPluginLoaderTest(unittest.TestCase):
    ...
    def test_should_raise_exception_when_requiring_plugin_and_plugin_is_not_found(
        ...
        load.assert_has_calls(
            [call("pybuilder.plugins.spam_plugin", "spam"), call("spam", "spam")]
        )
