...
class ImportTransformerTests(TestCase):
    ...
    def get_transformed_source(
        self, source, source_path, vendorized_path, vendorized_packages
    ):
        parsed_ast = ast.parse(source, filename=source_path)
        it = ImportTransformer(
            source_path, source, vendorized_path, vendorized_packages, []
        )
        it.visit(parsed_ast)
