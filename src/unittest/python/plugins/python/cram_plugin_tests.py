...
class CramPluginTests(unittest.TestCase):
    ...
    def test_running_plugin_cram_from_target(
        self, read_file_mock, report_mock, find_files_mock, command_mock
    ):
        project = Project(".")
    ...
    @patch("pybuilder.plugins.python.cram_plugin._report_file")
    @patch("pybuilder.plugins.python.cram_plugin.read_file")
    def test_running_plugin_from_scripts(
        self, read_file_mock, report_mock, find_files_mock, command_mock
    ):
        project = Project(".")
    ...
    def test_running_plugin_fails(
        ...
        expected_error_calls = [
            call("Cram tests failed! See report_file for full details:\ntail data"),
        ]
        self.assertEqual(expected_info_calls, logger.info.call_args_list)
    ...
    @patch("pybuilder.plugins.python.cram_plugin._report_file")
    @patch("pybuilder.plugins.python.cram_plugin.read_file")
    def test_running_plugin_no_failure_no_tests(
        self, read_file_mock, report_mock, find_files_mock, command_mock
    ):
        project = Project(".")
        project.set_property("verbose", True)
    ...
    @patch("pybuilder.plugins.python.cram_plugin._report_file")
    @patch("pybuilder.plugins.python.cram_plugin.read_file")
    def test_running_plugin_failure_no_tests(
        self, read_file_mock, report_mock, find_files_mock, command_mock
    ):
        project = Project(".")
        project.set_property("verbose", True)
