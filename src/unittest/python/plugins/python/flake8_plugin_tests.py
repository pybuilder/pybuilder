...
class FlakePluginInitializationTests(TestCase):
    ...
    def test_should_set_dependency(self):
        ...
        mock_project.plugin_depends_on.assert_called_with("flake8", "~=4.0")
    ...
    def test_should_leave_user_specified_properties_when_initializing_plugin(self):
        ...
        for property_name, property_value in expected_properties.items():
            self.assertEqual(self.project.get_property("flake8_break_build"), True)
            self.assertEqual(self.project.get_property("flake8_max_line_length"), 80)
            self.assertEqual(
            ...
            self.assertEqual(self.project.get_property("flake8_include_scripts"), True)
            self.assertEqual(self.project.get_property("flake8_max_complexity"), 10)
