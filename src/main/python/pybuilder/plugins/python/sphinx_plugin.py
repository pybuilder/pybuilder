...
SPHINX_PYB_CONFIG_FILE_PATH = SPHINX_PYB_RUNTIME_DIR + (
    SPHINX_PYB_CONFIG_MODULE + ".py",
)
...
def initialize_sphinx_plugin(project):
    ...
    project.set_property_if_unset("sphinx_output_per_builder", False)
    project.set_property_if_unset("sphinx_doc_author", default_doc_author)
    project.set_property_if_unset("sphinx_doc_builder", SPHINX_DOC_BUILDER)
    project.set_property_if_unset("sphinx_project_name", default_project_name)
    project.set_property_if_unset("sphinx_project_version", default_project_version)
    project.set_property_if_unset("sphinx_run_apidoc", False)

    # Extra arguments such as -d, -e, -M etc to add to Sphinx apidoc run
    project.set_property_if_unset("sphinx_apidoc_extra_args", [])

    # Extra arguments such as -E, -D etc to add to Sphinx build run
    project.set_property_if_unset("sphinx_build_extra_args", [])
...
def assert_sphinx_is_available(project, logger, reactor):
    ...
    reactor.pybuilder_venv.verify_can_execute(
        ["sphinx-build", "--version"], "sphinx-build", "plugin python.sphinx"
    )
    reactor.pybuilder_venv.verify_can_execute(
        ["sphinx-apidoc", "--version"], "sphinx-apidoc", "plugin python.sphinx"
    )
...
def run_sphinx_build(build_command, task_name, logger, project, reactor, builder=None):
    ...
    exit_code = reactor.pybuilder_venv.execute_command(
        build_command, log_file, shell=False
    )
    if exit_code != 0:
...
def sphinx_pyb_quickstart_generate(project, logger, reactor):
    ...
    """
    sphinx_quickstart_generate(
        project, logger, reactor
    )  # If this fails we won't touch the config directory further
...
""" % {
    ...
    source_apidoc_link = project.expand_path(
        "$sphinx_source_dir", SPHINX_PYB_APIDOC_DIR
    )
...
def get_sphinx_quickstart_command(project):
    ...
    """
    options = [
        _get_sphinx_launch_cmd("sphinx.cmd.quickstart", "main", "sphinx-quickstart"),
        "-q",
        "-p",
        project.get_property("sphinx_project_name"),
        "-a",
        project.get_property("sphinx_doc_author"),
        "-v",
        project.get_property("sphinx_project_version"),
...
def get_sphinx_build_command(project, logger, builder):
    ...
    options = [
        _get_sphinx_launch_cmd("sphinx.cmd.build", "main", "sphinx-build"),
        "-b",
        builder,
    ]
    ...
    if len(
        as_list(project.get_property("sphinx_doc_builder"))
    ) > 1 or project.get_property("sphinx_output_per_builder"):
...
def get_sphinx_apidoc_command(project, reactor):
    ...
    try:
        ...
        if reactor.pybuilder_venv.version[:2] >= (3, 3) and sphinx.version_info[:2] >= (
            1,
            5,
        ):
    ...
    options = [
        _get_sphinx_launch_cmd("sphinx.ext.apidoc", "main", "sphinx-apidoc"),
        "-H",
        project.get_property("sphinx_project_name"),
    ]
...
def generate_sphinx_pyb_runtime_config(project, logger):
    ...
    logger.debug(
        "Generating PyB-based Sphinx runtime config at %s", sphinx_pyb_conf_path
    )
...
def _get_sphinx_launch_cmd(module, func, script_name):
    return (
        "import sys; from %(module)s import %(func)s; "
        "sys.argv[0] = %(script_name)r; sys.exit(%(func)s())"
        % dict(module=module, func=func, script_name=script_name)
    )
