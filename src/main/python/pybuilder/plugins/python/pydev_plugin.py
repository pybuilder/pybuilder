...
from pybuilder.core import description, init, task

_DOT_PROJECT_TEMPLATE = string.Template(
...
_DOT_PYDEVPROJECT_PATH_LINE_TEMPLATE = string.Template(
    "\t\t<path>/$project_name/$path</path>\n"
)

_DOT_PYDEVPROJECT_TEMPLATE = string.Template(
...
def pydev_generate(project, logger):
    ...
    for path in paths:
        if os.path.exists(path):
            placeholders = {"project_name": project.name, "path": path}
            paths_string += _DOT_PYDEVPROJECT_PATH_LINE_TEMPLATE.substitute(
                placeholders
            )
