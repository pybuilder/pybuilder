...
from pybuilder.utils import Timer, discover_files_matching, read_file
...
def initialize_integrationtest_plugin(project):
    ...
    )  # deprecated, use integrationtest_file_glob.
...
def coverage_init(project, logger, reactor):
    ...
    if em.is_task_in_current_execution_plan(
        ...
        project.set_property(
            "it_coverage_python_env", project.get_property("integrationtest_python_env")
        )
...
def run_integration_tests_in_parallel(project, logger):
    ...
    for i in range(worker_pool_size):
        p = ctx.Process(
            target=pick_and_run_tests_then_report,
...
def discover_integration_tests_for_project(project, logger=None):
    ...
    if integrationtest_suffix is not None:
        ...
        project.set_property(
            "integrationtest_file_glob", "*{0}".format(integrationtest_suffix)
        )
...
def add_additional_environment_keys(env, project):
    ...
    if not isinstance(additional_environment, dict):
        raise ValueError(
            "Additional environment %r is not a map." % additional_environment
        )
...
def run_single_test(
    logger, project, reactor, reports_dir, test, output_test_names=True
):
    ...
    * the amount of workers running in parallel.
    ...
    def render(self):
        ...
        return "[%s%s%s%s]%s" % (
            ...
            pacman,
            running_tests_progress,
            waiting_tasks_progress,
            trailing_space,
        )
    ...
    def render_to_terminal(self):
        if self.can_be_displayed:
            ...
            text_to_render = "%s%s" % (
                characters_to_be_erased * self.BACKSPACE,
                text_to_render,
            )
