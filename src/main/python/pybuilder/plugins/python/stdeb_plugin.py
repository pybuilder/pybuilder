...
def initialize_make_deb_plugin(project):
    ...
    project.set_property_if_unset("deb_package_maintainer", DEB_PACKAGE_MAINTAINER)
    project.set_property_if_unset("path_final_build", PATH_FINAL_BUILD)
    project.set_property_if_unset("path_to_source_tarball", PATH_TO_SOURCE_TARBALL)
...
@task(
    "make_deb",
    "converts a source tarball into a Debian source package and build a .deb package",
)
@depends("publish")
def py2dsc_deb(project, logger, reactor):
    ...
    run_py2dsc_deb_build(
        reactor.pybuilder_venv, build_command, task_name, logger, project
    )
...
def get_py2dsc_deb_command(project):
    ...
    return [
        "py2dsc-deb",
        "--maintainer",
        project.get_property("deb_package_maintainer"),
        "-d",
        project.get_property("path_final_build"),
