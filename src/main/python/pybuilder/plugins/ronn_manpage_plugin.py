...
from pybuilder.core import after, depends, description, init, task, use_plugin
...
def generate_manpages(project, logger, reactor):
    ...
    reactor.pybuilder_venv.execute_command(
        generate_manpages_command, ronn_report_file, shell=True
    )


def build_generate_manpages_command(project):
    ronn_pipe_command = "ronn -r --pipe %s" % project.get_property("manpage_source")
    compressed_manpage_file = "%s.%d.gz" % (
        project.name,
        project.get_property("manpage_section"),
    )
    compress_command = "gzip -9 > %s" % os.path.join(
        project.get_property("dir_manpages"), compressed_manpage_file
    )
