...
def collect_project_information():
    ...
    project_name = (
        prompt_user("Project name", default_project_name) or default_project_name
    )
    ...
    dir_source_main_scripts = prompt_user(
        "Scripts directory", DEFAULT_SCRIPTS_DIRECTORY
    )
...
def suggest_plugins(plugins):
    chosen_plugins = [
        plugin for plugin in [suggest(plugin) for plugin in plugins] if plugin
    ]
...
class PythonProjectScaffolding():
...
"""
    ...
    def build_imports(self):
        self.activated_plugins = "\n".join(
            ['use_plugin("%s")' % plugin for plugin in self.plugins]
        )
    ...
    def build_initializer(self):
        ...
        if not self.is_default_source_main_python:
            properties_to_set.append(
                ("dir_source_main_python", self.dir_source_main_python)
            )
        if not self.is_default_source_unittest_python:
            properties_to_set.append(
                ("dir_source_unittest_python", self.dir_source_unittest_python)
            )
        if not self.is_default_source_main_scripts:
            properties_to_set.append(
                ("dir_source_main_scripts", self.dir_source_main_scripts)
            )
        if not self.is_default_docs:
            properties_to_set.append(("dir_docs", self.dir_docs))

        initializer_body = self._build_initializer_body_with_properties(
            properties_to_set
        )
    ...
    def set_up_project(self):
        for needed_directory in (
            self.dir_source_main_python,
